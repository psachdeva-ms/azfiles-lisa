...
class KernelSourcePackager(DeploymentTransformer):
    ...
    def _internal_run(self) -> Dict[str, Any]:
        ...
        self._log.info(
            f"use_cache value: {runbook.use_cache} (type: {type(runbook.use_cache)})"
        )
            ...
            if (
                ...
                for package_path in package_paths:
                    if (
                        ...
                        and node.execute(f"test -f{package_path}", shell=True).exit_code
                        == 0
                    ):
                        return True
                return False
    ...
    def _update_cache(
        ...
        if metadata:
            ...
            if len(cache) > max_cache_size:
                removed = cache.pop()
                self._log.info(
                    f"Cache full. Removed oldest entry: \
        ...
        if not package_paths:
            raise Exception(
                "No package_paths found in cache for the given \
            commit_id."
            )
...
class RepoWorktree(BaseLocation):
    ...
    def get_source_code(self) -> PurePath:
        ...
        if not self._node.shell.exists(code_path):
            self._log.debug(f"creting dir: {code_path}")
            self._node.execute(f"mkdir -p {code_path}", shell=True)
            self._node.execute(f"chmod 0777 {code_path}", shell=True)

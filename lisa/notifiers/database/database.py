...
from lisa import messages, notifier, schema  # type: ignore
from lisa.testsuite import TestResultMessage, TestStatus
from lisa.util import LisaException, hookimpl, plugin_manager

from .common import get_build_url, get_extra_information, get_test_run_log_location
...
class LsgDatabase(DatabaseMixin, notifier.Notifier):  # type: ignore
    """
    ...
    def _initialize(self, *args: Any, **kwargs: Any) -> None:
        ...
        # self._test_ai_analysis_cache: Dict[str, Any] = dict()
        ...
        # self.TestAiAnalysis = self.base.classes.TestAiAnalysis
    ...
    def _add_test_result(self, message: TestResultMessage) -> None:
        ...
        # if message.id_ not in self._test_ai_analysis_cache:
        #     test_result_id = getattr(
        #         self._test_results_cache.get(message.id_), "Id", None
        #     )
        #     ai_analysis = message.analysis.get("AI", "")
        #     if test_result_id and ai_analysis:
        #         analysis = self.TestAiAnalysis(
        ...
        #         self._test_ai_analysis_cache[message.id_] = analysis
        #         self._log.debug("inserting test AI analysis into database...")
        #         self.commit_and_close_session(session)
    ...
    def _process_test_result_message(self, message: TestResultMessage) -> None:
        ...
        if message.status == TestStatus.QUEUED:
            ...
            # analysis = self._test_ai_analysis_cache.get(message.id_, None)
            # ai_analysis = message.analysis.get("AI", "")

            @retry(tries=5, delay=1, backoff=2)  # type: ignore
            def add_with_retry() -> None:
                session = self.create_session()
                session.add(result)
                # if analysis and ai_analysis:
